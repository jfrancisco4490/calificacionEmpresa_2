...
module.exports = {
	...
	// Permite el registro de un nuevo usuario o alumno
	// **Parámetros de Entrada:**
	// - **in_db**: identificador de la conexión a la base de datos
	// - **in_usr**: identificador del usuario o alumno en la aplicación (único)
	// - **in_nombre**: nombre completo del alumno
	// - **in_apellido**: apellidos completos del alumno
	// - **in_extra**: información complementaria o adicional del usuario
	// **Salida:** _true_ si el registro del nuevo alumno/usuario se ejecutó correctamente, _false_ si no
	crearAlumno : function(in_db,in_usr,in_nombre,in_apellido,in_extra){
		in_db.run("INSERT INTO ALUMNO(usuario, nombres, apellidos, extra) VALUES ('"+in_usr+"','"+in_nombre+"','"+in_apellido+"','"+in_extra+"')", function(err){
	...
	// Registrar nueva calificación o valoración de un usuario (estudiante) a una empresa
	// **Parámetros de Entrada:**
	// - **in_db**: identificador de la conexión a la base de datos
	...
	// Modificar calificación o valoración de un usuario (estudiante) a una empresa
	// **Parámetros de Entrada:**
	// - **in_db**: identificador de la conexión a la base de datos
	// - **in_id**: identificador de la empresa calificada
	// - **in_usr**: nombre de usuario del estudiante que realizó la valoración
	// - **in_valor**: nueva calificación numérica (1 a 5) para la empresa
	// **Salida:** _true_ si la actualización de la calificación se ejecutó correctamente, _false_ si no
	modificarCalificacion : function(in_db, in_id, in_usr, in_valor){
 
		in_db.run("UPDATE CALIFICACION SET valor = '"+in_valor+"' WHERE usuario = '"+in_usr+"' AND identificador = '"+in_id+"'", function(err) {
